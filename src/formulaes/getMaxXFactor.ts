...
export default function getMaxXFactor(currencyPair: CurrencyPair): number{
    const maxDiff = Number(currencyPair.value) * CurrencyPairCircuitLimits.upperLimit;
    const tickSize = CurrnecyPairTickSizes[currencyPair.id as keyof typeof CurrnecyPairTickSizes];
