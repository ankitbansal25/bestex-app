 ...
 */
export async function getLastDayCurrencyPairClosePrices(onSuccess: ((data: Array<CurrencyPair>)=> void), onError: ((error: any) => void)) {
    fetchLastDayCurrencyPairClosePrices().then((data) => { onSuccess(data); }).catch((error) => { onError(error)})
}
